...
## Exposición a Home Assistant

Si el usuario tiene HA en una raspberry (caso real para esta familia de monitores), el patrón ya validado en `gpu_monitor` es: **paquete YAML que usa el componente `rest` de `default_config`** + **túnel SSH forward desde la raspberry** al host del monitor. Cero custom components, cero Python en HA.

### Topología obligada

```
[ host del monitor ]                      [ raspberry, HA Docker --network host ]
  <monitor>d :PUERTO  ◄── ssh -L ───────  127.0.0.1:PUERTO
                                                 │
                                                 └─► sensor.rest (poll 15 s)
```

**Forward desde la raspberry**, NO reverse desde el host. La raspberry es la máquina always-on con HA y otros servicios — el túnel vive donde viven los demás. El host del monitor solo añade una pubkey en `authorized_keys`. Cero servicios nuevos en el host (importante porque cada monitor de la familia añadiría su propio túnel si fuera reverse — escalado pésimo).

### Componente `rest` de HA — sintaxis

Un solo `resource:` con `scan_interval: 15` y N `sensor:` declarados como `value_template: "{{ value_json.<path> }}"`. Una request HTTP por ciclo, multiplexada en N entidades. Para campos que pueden faltar (ej. GPU index 1 cuando solo hay 1 GPU):
...
`None` se traduce a `unavailable` en HA — se ve en el UI sin romper la entidad. `device_class: temperature/power/data_size` + `state_class: measurement` para que HA grabe historial y los gauges/graphs lo entiendan.

### Gotcha SSH que costó tiempo: `restrict` mata el port forwarding

`restrict,permitopen="127.0.0.1:PUERTO" ssh-ed25519 ...` parece OK pero **falla con `administratively prohibited: open failed`**. El cliente conecta, autentica, abre el listener local, y al cruzar al host sshd corta el canal.

`restrict` (OpenSSH ≥ 7.6) deshabilita TODO. `permitopen` solo *limita* el destino del forwarding pero NO lo *reactiva*. La línea correcta:

```
restrict,port-forwarding,permitopen="127.0.0.1:PUERTO" ssh-ed25519 AAAA... <comment>
```

Si haces el reverse (desde el host hacia la pi), simétrico: `restrict,port-forwarding,permitlisten="PUERTO"`. **Sin `port-forwarding` explícito, todo lo demás es decorativo.**

### User systemd unit + linger en la pi

`~/.config/systemd/user/<monitor>-ha-tunnel.service` con:

```ini
ExecStart=/usr/bin/ssh -N -T \
    -i %h/.ssh/id_ed25519_<monitor>_tunnel \
    -o IdentitiesOnly=yes \
    -o ServerAliveInterval=30 \
    -o ServerAliveCountMax=3 \
    -o ExitOnForwardFailure=yes \
    -o StrictHostKeyChecking=accept-new \
    -L 127.0.0.1:PUERTO:127.0.0.1:PUERTO \
    <user>@<host>
Restart=always
RestartSec=10
```

Una sola vez: `sudo loginctl enable-linger <pi-user>`. Sin linger, el servicio user se cae al cerrar sesión SSH y nunca arranca tras reboot si nadie hace login.

`accept-new`, no `yes`: la primera conexión añade el host a `known_hosts` automáticamente, pero rechaza si la huella *cambia* (que es lo que quieres detectar — un MITM, no un host nuevo). Sin esto, el túnel cuelga esperando un "yes" interactivo que nadie va a teclear.

### Clave dedicada, no la personal

`ssh-keygen -t ed25519 -N "" -f ~/.ssh/id_ed25519_<monitor>_tunnel -C "<pi-host>-<monitor>-tunnel"`. Sin passphrase porque va a usarse desde un servicio 24/7, restringida en `authorized_keys` para que solo pueda port-forwardear al puerto del monitor (nada más: ni shell, ni X11, ni otros puertos). La clave personal del usuario suele tener passphrase y mezclarla con servicios es asking for trouble.

### HA en Docker con `--network host`

`docker inspect homeassistant` debe mostrar `NetworkMode=host`. Por eso `127.0.0.1:PUERTO` desde dentro del contenedor llega al loopback de la pi y al túnel. Si HA estuviera en bridge, habría que usar `host.docker.internal` (no funciona en linux por default) o la IP de docker0 (`172.17.0.1`). Es la situación más común porque HA OS / supervised viven en host net.

### REST y no SSE — mismo razonamiento

HA tiene `rest` declarativo nativo: una única request compartida por N sensores. SSE necesitaría custom component. A 15 s × ~600 B = irrelevante. **No te metas en custom components solo por purismo de protocolo.**

### `configuration.yaml` y packages

Una vez por instalación de HA, añadir al final del `configuration.yaml`:
...
A partir de ahí cada monitor deja su `<monitor>.yaml` en `/config/packages/` y HA lo carga en frío sin tocar el config principal. Habrá un package por monitor (gpu, cpu, ram, disk, audio in/out) y todos comparten el mismo include.

### Puerto y replicación de schema

- **Puerto del túnel** = puerto del daemon (gpu=9123, cpu=9124, ram=9125, ...). Un túnel por monitor — son baratos (~1 MB cada uno) y aislan fallos. Alternativa: un solo túnel con N `-L` flags, pero un fallo de uno tira el otro.
- **Si añades un campo** a `<monitor>_core::Snapshot`, replícalo en `home-assistant/packages/<monitor>.yaml` como nuevo `value_template`. Igual que la replicación a `front-mac/Models.swift`. El package YAML asume el shape del JSON; un campo nuevo en Rust no aparece automáticamente en HA.

### `/healthz` vive en raíz, no bajo `/v1`

`GET /v1/healthz` → 404. `GET /healthz` → 200. Solo importa para verificación manual del túnel (`curl` desde la pi); el package YAML usa `/v1/snapshot`. Documenta esto si haces troubleshooting docs — gasté 30 segundos pensando que el túnel estaba roto cuando solo era el path equivocado.

### Layout del directorio

```
home-assistant/
├── README.md                              # arquitectura + deploy
├── packages/<monitor>.yaml                # paquete HA (drop-in)
├── lovelace/<monitor>_dashboard.yaml      # vista sections+grid+gauges
└── tunnel/
    ├── <monitor>-ha-tunnel.service        # user systemd unit
    └── install.sh                         # genera clave + instala unit
```

Patrón ya pisado: el `install.sh` debe imprimir la línea exacta para `authorized_keys` con las restricciones — si dejas que el usuario las redacte a mano, olvida `port-forwarding` y termina debugging "administratively prohibited" como yo.

### Lovelace en formato `sections`, no `cards:` legacy

HA 2024+ usa `views[].type: sections` con secciones `type: grid` que contienen `tile`, `heading`, `gauge`, `history-graph`. La versión vieja (`views[].type` ausente, `cards:` directo) sigue funcionando pero se ve mal en el UI moderno y el Raw editor da errores de validación al pegar. Para el archivo del repo escribe el formato moderno desde el inicio. Pegar como entrada bajo `views:`, no como root del dashboard.
...
- `home-assistant/packages/gpu_monitor.yaml` — package HA con 28 sensores REST y la sintaxis `value_template` con guardas de longitud para GPUs opcionales.
- `home-assistant/tunnel/gpu-monitor-ha-tunnel.service` — user systemd unit del túnel forward, con la clave dedicada y todos los flags inline.
- `home-assistant/tunnel/install.sh` — generación de clave + impresión de la línea exacta de `authorized_keys` con `restrict,port-forwarding,permitopen=...`.
